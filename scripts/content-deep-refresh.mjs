// Quarterly deep content refresh: DeepSeek V4-Pro
...
async function main() {
  if (!DEEPSEEK_API_KEY) {
    console.error('[deep-refresh] DEEPSEEK_API_KEY not set');
    process.exit(1);
  }
        ...
        const response = await fetch('https://api.deepseek.com/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: `Rewrite this tinnitus article in Kalesh's voice. Cross-traditional references (Buddhism, Taoism, Vedanta, neuroscience). Intellectual warmth, observer-humor.
...
Return HTML body only.`
  ...
  if (GH_PAT) {
    try {
      ...
      execSync(`cd ${ROOT} && git add content/ && git commit -m "Deep refresh: ${refreshed} articles" && git push`, {
        stdio: 'inherit',
        env: { ...process.env, GIT_AUTHOR_NAME: EDITORIAL_NAME, GIT_COMMITTER_NAME: EDITORIAL_NAME }
      });
      console.log('[deep-refresh] Pushed to GitHub');
    } catch (e) {
